...
namespace Mkb.DapperRepo.Tests.Tests.ReflectionUtilsTests
{
    public class GetEntityPropertyInfoTests
    {
        ...
        [Test]
        public void Ensure_Throw_error_if_missing_primary_key()
        {
            var entityPropertyInfo = Assert.Throws<PrimaryKeyNotFoundException>(() =>
                ReflectionUtils.GetEntityPropertyInfo<TableWithNoPrimaryKey>()
            );

            Assert.AreEqual("Primary key not found on table: TableWithNoPrimaryKey", entityPropertyInfo.Message);
        }
        ...
        {
            [PrimaryKey] [SqlColumnName(ColName)] public string ColNameChange { get; set; }
        }


        private class TableWithNoPrimaryKey
        {
