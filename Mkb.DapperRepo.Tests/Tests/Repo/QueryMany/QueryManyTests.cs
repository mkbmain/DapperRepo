...
namespace Mkb.DapperRepo.Tests.Tests.Repo.QueryMany
{
    public class QueryManyTests : BaseSyncTestClass
    {
        ...
        [Test]
        public void Ensure_we_Get_correct_record_back_query()
        {
            var testTableItems = new[]
            {
                new TableWithNoAutoGeneratedPrimaryKey {Id = Guid.NewGuid().ToString("N"), NameTest = "Wanted1", SomeNumber = 33},
            ...
            var items = (Sut.QueryMany<TableWithNoAutoGeneratedPrimaryKey>(
                "select * from TableWithNoAutoGeneratedPrimaryKey where SomeNumber = @num",new {num = 33})).ToArray();
            var expected = testTableItems.Where(f => f.SomeNumber == 33);
            Assert.AreEqual(expected.Count(), items.Length);
            foreach (var item in expected)
            {
        ...
        [Test]
        public void Ensure_we_Get_correct_record_back_diff_name_param()
        {
            var testTableItems = new[]
            {
                new TableWithNoAutoGeneratedPrimaryKey {Id = Guid.NewGuid().ToString("N"), NameTest = "Wanted1", SomeNumber = 33},
