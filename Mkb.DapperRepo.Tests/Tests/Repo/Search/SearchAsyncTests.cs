...
namespace Mkb.DapperRepo.Tests.Tests.Repo.Search
{
    public class SearchAsyncTests : BaseAsyncTestClass
    {
        ...
        public async Task Ensure_we_can_match_null()
        {
            ...
            {
                new TableWithNoAutoGeneratedPrimaryKey {Id = Guid.NewGuid().ToString("N"), NameTest = "Michael", SomeNumber = 33},
                new TableWithNoAutoGeneratedPrimaryKey {Id = Guid.NewGuid().ToString("N"), NameTest = "Michael", SomeNumber = null}
            };

            DataBaseScriptRunnerAndBuilder.InsertTableWithNoAutoGeneratedPrimaryKey(Connection, testTableItems);

            var result = await Sut.Search(new TableWithNoAutoGeneratedPrimaryKey {NameTest = "Michael", SomeNumber = null}, 
                new []
                {
                    new SearchCriteria
                    {
        ...
        public async Task Ensure_we_can_match_on_numbers()
        {
            ...
            var result = await Sut.Search<TableWithNoAutoGeneratedPrimaryKey,int?>(nameof(TableWithNoAutoGeneratedPrimaryKey.SomeNumber),33, SearchType.Equals);
