...
namespace Mkb.DapperRepo.Tests.Tests.Repo.AddMany
{
    ...
    {
        public AddManyAsyncTests() : base($"{nameof(AddManyAsyncTests)}")
        {
        }
        ...
        [Test]
        public async Task Ensure_we_can_add_multiple_records_diff_name()
        {
            var testTableItems = new[]
            {
                new TableWithNoAutoGeneratedPrimaryKey {Id = Guid.NewGuid().ToString("N"), Name = "Michale", SomeNumber = 33},
            ...
            await Sut.AddMany(testTableItems.Select(f => new TableWithNoAutoGeneratedPrimaryKeyDiffSqlName(f)));
        ...
        [Test]
        public async Task Ensure_we_can_add_multiple_records_auto_increment()
        {
            var testTableItems = new[]
            {
        ...
        [Test]
        public async Task Ensure_we_can_add_multiple_records_auto_increment_with_diff_name()
        {
            var testTableItems = new[]
            {
                new TableWithAutoIncrementPrimaryKey {Name = "Michale", SomeNumber = 33},
            ...
            await Sut.AddMany(testTableItems.Select(f => new TableWithAutoIncrementPrimaryKeyDiffSqlName(f)));
