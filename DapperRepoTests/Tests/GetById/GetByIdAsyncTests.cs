...
namespace DapperRepoTests.Tests.GetById
{
    public class GetByIdAsyncTest : BaseAsyncTestClass
    {
        ...
        public async Task Ensure_we_Can_Get_correct_record_Back_when_id_is_a_number()
        {
            ...
            var item = await Sut.GetById(new TableWithAutIncrementPrimaryKey {Id = record.Id});
            Assert.IsNotNull(item);
        ...
        public async Task Ensure_if_no_record_found_we_return_null()
        {
            ...
            var item = await Sut.GetById(new TableWithNoAutoGeneratedPrimaryKey {Id = Guid.NewGuid()});
            Assert.IsNull(item);
        }
    }
}