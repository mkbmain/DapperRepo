...
namespace DapperRepoTests.Utils
{
    ...
    {
        public static void InsertTableWithNoAutoGeneratedPrimaryKey(string connection, IEnumerable<TableWithNoAutoGeneratedPrimaryKey> testTables)
        {
            var sql =
        ...
        public static void InsertTableWithAutoGeneratedPrimaryKey(string connection, IEnumerable<TableWithAutIncrementPrimaryKey> testTables)
        {
            var sql =
                $"Insert into {nameof(TableWithAutIncrementPrimaryKey)} (name,SomeNumber) values{string.Join(",", testTables.Select(f => $"('{f.Name}',{f.SomeNumber})"))}";
            ExecuteCommandNonQuery(connection, sql);
        }
        ...
        public static IEnumerable<T> GetAll<T>(string connection) where T : class, new()
        {
            ...
            foreach (var record in sqlDataReader)
            {
                ...
                for (int i = 0; i < dataRecord.FieldCount; i++)
                {
                    ...
                    propertyInfo.SetValue(item, Convert.ChangeType(dataRecord[i], Nullable.GetUnderlyingType(propertyInfo.PropertyType)?? propertyInfo.PropertyType), null);
                }
